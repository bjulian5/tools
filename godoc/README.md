...
executable lives at github.com/bjulian5/tools/cmd/godoc.
...
```
godoc -templates=$GOPATH/src/github.com/bjulian5/tools/godoc/static -http=:6060
```
...
2) Run `go generate github.com/bjulian5/tools/godoc/static` so `static/static.go` picks
up the change.

3) Run `go install github.com/bjulian5/tools/cmd/godoc` so the compiled `godoc` binary
picks up the change.
